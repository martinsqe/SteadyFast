...
const EXPERTISE = [
  ...
  { id: "Hybrid & Electric Propulsion Systems",label: "Hybrid / EV",  icon: "🔋", desc: "Motors & battery systems" },
  { id: "Suspension & Wheel Dynamics",         label: "Chassis",      icon: "🏁", desc: "Suspension & braking" },
  { id: "Beginner",                            label: "Beginner",     icon: "🛠️", desc: "Entry level professional" },
  { id: "Intermediate",                        label: "Intermediate", icon: "🔧", desc: "Solid foundation" },
  { id: "Expert",                              label: "Expert",       icon: "🌟", desc: "Veteran technician" },
  { id: "Master",                              label: "Master",       icon: "👑", desc: "Top-tier diagnostic" },
];

const S = {
  page: {
    display: "flex", gap: "1.5rem", color: "#fff",
  ...
  tabBtn: (active) => ({
    ...
    background: active ? "rgba(16,185,129,0.06)" : "none",
    border: "none",
  ...
  expCard: (active) => ({
    ...
    padding: "0.85rem 0.9rem", borderRadius: "12px", cursor: "pointer",
    ...
    boxShadow: active ? "0 0 0 1px rgba(16,185,129,0.15)" : "none",
  ...
  expName: (active) => ({
    ...
    color: active ? "#10b981" : "#fff", marginBottom: "0.1rem",
  }),
  expDesc: { fontSize: "0.72rem", color: "rgba(255,255,255,0.38)" },
  tick: {
...
export default function MechanicProfile() {
  ...
  const [form, setForm] = useState({
    ...
    dateOfBirth: "", expertiseLevel: "Beginner",
  ...
  useEffect(() => {
    if (!user) return;
    let dob = "";
    try { if (user.dateOfBirth) dob = new Date(user.dateOfBirth).toISOString().split("T")[0]; } catch (_) {}
    setForm({
      name: user.name || "",
      email: user.email || "",
      phone: user.phone || "",
    ...
    if (user.profileImage) {
      setPreview(user.profileImage.startsWith("http") ? user.profileImage : `${BASE}${user.profileImage}`);
    }
  }, [user]);
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (tab === "personal") {
        ...
        data.append("expertiseLevel", form.expertiseLevel);
      } else {
  ...
  const cur = EXPERTISE.find((x) => x.id === form.expertiseLevel);
  const TABS = [
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.sidebar}>
        ...
        <span style={S.rolePill}>Mechanic</span>
        ...
        <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "0.4rem", marginTop: "0.5rem" }}>
          ...
          {user.expertiseLevel && (
            <div style={S.infoRow}><span>🏷️</span><span>{user.expertiseLevel}</span></div>
          )}
        </div>
      </div>
        ...
        <form style={S.body} onSubmit={save}>
          ...
          {tab === "personal" && (
            <div>
              ...
              <p style={S.sectionDesc}>Update your name, contact details and date of birth.</p>
              <div style={S.grid2}>
                <div>
                  <label style={S.label}>Full Name</label>
                  <input style={S.input} name="name" value={form.name} onChange={set} required placeholder="John Doe" />
                </div>
                <div>
                  <label style={S.label}>Email Address</label>
                  <input style={S.input} type="email" name="email" value={form.email} onChange={set} required placeholder="john@example.com" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input style={S.input} type="date" name="dateOfBirth" value={form.dateOfBirth} onChange={set} />
                </div>
          ...
          {tab === "professional" && (
            <div>
              <div style={S.sectionTitle}>Professional Expertise</div>
              <p style={S.sectionDesc}>Select your specialization so clients can find the right mechanic.</p>
              <div style={{ fontSize: "0.73rem", fontWeight: 700, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "0.75rem" }}>
                Specialties
              </div>
              <div style={S.expGrid}>
                {EXPERTISE.slice(0, 6).map((opt) => {
                  const active = form.expertiseLevel === opt.id;
                  return (
                    <div key={opt.id} style={S.expCard(active)} onClick={() => setForm((p) => ({ ...p, expertiseLevel: opt.id }))}>
                      <div style={S.expIcon(active)}>{opt.icon}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={S.expName(active)}>{opt.label}</div>
                        <div style={S.expDesc}>{opt.desc}</div>
                      </div>
                      {active && <div style={S.tick}>✓</div>}
                    </div>
                  );
                })}
              </div>
              ...
              <div style={S.pwTips}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  {[
                    ...
                    <li key={txt} style={{ fontSize: "0.82rem", color: met ? "#10b981" : "rgba(255,255,255,0.28)", display: "flex", gap: "0.45rem" }}>
                      <span>{met ? "●" : "○"}</span>{txt}
                    </li>
                  ))}
                </ul>
